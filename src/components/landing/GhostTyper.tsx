...
export const GhostTyper = ({ text, isActive, visualScale }: GhostTyperProps) => {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="w-full px-4 py-2 font-mono tracking-normal sm:px-6"
    >
      <div
        className="mx-auto w-full max-w-[1320px] rounded-lg border border-cyan-400/30 bg-black/50 px-5 py-3 backdrop-blur-sm sm:px-8 sm:py-3.5"
        style={{
      ...
      >
        ...
        <div
          className="min-h-[2.2em] font-mono leading-tight text-cyan-300"
          style={{
            textShadow: '0 0 12px rgba(34,211,238,0.5)',
            fontSize: `clamp(1.15rem, ${2.3 * textScale}vw, ${2.9 * textScale}rem)`,
          }}
        >
